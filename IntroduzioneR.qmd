...
### Il dataset `CASchools`

Per avere un'idea dei dataset messi a disposizione da `AER` possiamo utilizzare la funzione di `help`
```{r}
#| eval: false
help(package="AER")
```

In questo corso utilizzeremo il dataset `CASchools` che proviene dal pacchetto `AER`. Il dataset è stato salvato in formato CSV nella cartella `data/` e può essere caricato direttamente:
```{r}
#| message: false
#| warning: false
...
Il data set può essere esplorato utilizzando la funzione `summary()`
```{r}
summary(CASchools)
```
...
```{r}
library(ggplot2)
# I dati CASchools sono già stati caricati in precedenza
